...
class Evaluator(object):
    ...
    def write_labels(self, fn):
        ...
        f.write(
            "\n".join(
                "{0} {1}".format(name, i) for i, name in enumerate(self.feat_order)
            )
        )
        f.close()
    ...
    def read_all_wiktionary(self):
        for lang in listdir(self.cfg["dumpdir"]):
            logging.info(lang)
            self.read_wiktionary(
                self.cfg["dumpdir"] + "/" + lang + "/" + self.cfg["word_pairs_outfile"]
            )
    ...
    def featurize_group(self, group):
        ...
        self.map_int_feature_to_binaries(
            "pivot_langs", pivot_langs, self.pivot_gran, feats
        )
        left_langs = set([lang for lang, _ in left])
        self.map_int_feature_to_binaries(
            "left_langs", len(left_langs), self.edge_gran, feats
        )
        right_langs = set([lang for lang, _ in right])
        self.map_int_feature_to_binaries(
            "right_langs", len(right_langs), self.edge_gran, feats
        )
        self.map_int_feature_to_binaries(
            "left_disjunct", len(left_langs - right_langs), self.edge_gran, feats
        )
        self.map_int_feature_to_binaries(
            "right_disjunct", len(right_langs - left_langs), self.edge_gran, feats
        )
        self.add_wc_features(pair, feats)

        self.map_int_feature_to_binaries_by_wc("_pivot", pivots, self.pivot_gran, feats)
        self.map_int_feature_to_binaries_by_wc("_left", left, self.edge_gran, feats)
        self.map_int_feature_to_binaries_by_wc("_right", right, self.edge_gran, feats)
        return feats, pair
    ...
    def map_int_feature_to_binaries_by_wc(self, pre, vals, granularity, feat_dict):
        ...
        for wc in self.wikicodes:
            self.map_int_feature_to_binaries(
                wc + pre, val_wc[wc], granularity, feat_dict
            )
    ...
    def map_int_feature_to_binaries(
        self, feat_prefix, val, granularity, feat_dict, mode="le"
    ):
