...
lot of new features.
"""
from wikt2dict.article_parsers import (
    DefaultArticleParser,
    LangnamesArticleParser,
...
class WiktionaryConfig(DictLikeClass):
    ...
    def dump_url(self):
        return (
            "http://dumps.wikimedia.org/{0}wiktionary"
            "/latest/{0}wiktionary-latest-pages-meta-current.xml.bz2".format(self.wc)
        )
        ...
        if self._skip_trans_re is None:
            ...
            if self.blacklist:
                self._skip_trans_re = re.compile(
                    r"(" + "|".join(self.blacklist) + ")", re.UNICODE
                )
            else:
                self._skip_trans_re = ""
...
class DefaultParserConfig(ParserConfig):
    ...
    def trad_re(self):
        if not self._trad_re:
            self._trad_re = re.compile(
                r"\{\{" + self.translation_prefix + r"\|([^}|]+)\|"  # wikicode
                r"([^}|]*)"  # word
                r"(?:\|([^}|]*))*\}\}",
                re.UNICODE,
            )  # rest
    ...
    def delimiter_re(self):
        if not self._delimiter_re:
            self._delimiter_re = re.compile(
                self.translation_entity_delimiter, re.UNICODE
            )
        ...
        if not self._section_parsers:
            ...
            if self._section_parser_configs:
                for (
                    ...
                    parser_cfg_cl,
                    parser_cfg,
                ) in self._section_parser_configs:
                    self._section_parsers.append(
                        (parser_cl(self, parser_cfg_cl(parser_cfg)))
                    )
...
class DefaultWiktionaryConfig(WiktionaryConfig):
    def __init__(self):
        ...
        try:
            ...
            cfg = {}
        self._parser_configs = [[DefaultArticleParser, DefaultParserConfig, cfg]]
...
class LangnamesWiktionaryConfig(WiktionaryConfig):
    def __init__(self):
        ...
        try:
            ...
            cfg = {}
        self._parser_configs = [[LangnamesArticleParser, LangnamesParserConfig, cfg]]
...
class ItalianConfig(LangnamesWiktionaryConfig):
    def __init__(self):
        ...
        self.langnames_cfg = {
            "langnames": False,
            "translation_line": r":\*\{{([^}]+)}}:\s*(.*)",
            "skip_translation_line": r"(\<\!\-\-\s*inserisci|inserisci.*traduzioni|altri\ template\ utili)",
            "skip_translation": r"(:\*:|<\!|^\?+$)",
        }
